...
const static char *TAG = "doubleTemp";
uint32_t r,g,b;
float temp_ntc = 0; // NTC温度值
...
void app_main(void)
{
	...
	while (1){
		...
		if (DHT11_StartGet(&temp, &hum)){
			ESP_LOGI(TAG, "temp->%i.%i C     hum->%i%%     ntc temp:%.2fC", temp / 10, temp % 10, hum,temp_ntc);
		}
		if (temp_ntc<21 || temp_ntc>26)
		{
			r = 5;
			g = 1;
			b = 1;
